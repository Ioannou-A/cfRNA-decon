...
output_dir="TOO-Decon-Published/Toden_AD/Decon-Results_Toden_AD"
...
cp TOO-Decon-Published/Toden_AD/All-AD_Counts_v46_Clean_UniquePatients_CPM.txt ${input_dir}
...
singularity exec -B ${input_dir}:/src/data -B ${output_dir}:/src/outdir fractions_latest.sif /src/CIBERSORTxFractions \
    --username "$CIBERSORTX_USERNAME" --token "$CIBERSORTX_TOKEN" --mixture /src/data/All-AD_Counts_v46_Clean_UniquePatients_CPM.txt --sigmatrix /src/data/${sig_name} --verbose TRUE
