...
cp TOO-Decon-Published/Chalasani_Fatty-Liver/All-NASH_Counts_v46_Clean_UniquePatients_CPM.txt ${input_dir}
...
singularity exec -B ${input_dir}:/src/data -B ${output_dir}:/src/outdir fractions_latest.sif /src/CIBERSORTxFractions \
    --username "$CIBERSORTX_USERNAME" --token "$CIBERSORTX_TOKEN" --mixture /src/data/All-NASH_Counts_v46_Clean_UniquePatients_CPM.txt --sigmatrix /src/data/${sig_name} --verbose TRUE
