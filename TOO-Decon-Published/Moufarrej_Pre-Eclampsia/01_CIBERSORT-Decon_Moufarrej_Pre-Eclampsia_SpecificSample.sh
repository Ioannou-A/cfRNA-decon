...
output_dir="TOO-Decon-Published/Moufarrej_Pre-Eclampsia/Decon-Results_Moufarrej_Pre-Eclampsia_SpecificSample"
...
cp TOO-Decon-Published/Moufarrej_Pre-Eclampsia/GSE192902_Pre-eclampsia_preQC_filtered_counts_UniquePatient_SpecificSample.txt ${input_dir}
...
singularity exec -B ${input_dir}:/src/data -B ${output_dir}:/src/outdir fractions_latest.sif /src/CIBERSORTxFractions \
    --username "$CIBERSORTX_USERNAME" --token "$CIBERSORTX_TOKEN" --mixture /src/data/GSE192902_Pre-eclampsia_preQC_filtered_counts_UniquePatient_SpecificSample.txt --sigmatrix /src/data/${sig_name} --verbose TRUE
