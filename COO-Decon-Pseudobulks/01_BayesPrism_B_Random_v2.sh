...
bulk_files=(
  "COO-Decon-Pseudobulks/Random_v2C_All-Counts.txt"
)
...
output_dir="COO-Decon-Pseudobulks/BayesPrism-Deconvolutions"
mkdir -p "$output_dir"
...
for bulk in "${bulk_files[@]}"; do
  for sc in "${sc_refs[@]}"; do
    ...
    Rscript COO-Decon-Pseudobulks/01_BayesPrism_A_Run.R "$bulk" "$sc" "$output_file"
