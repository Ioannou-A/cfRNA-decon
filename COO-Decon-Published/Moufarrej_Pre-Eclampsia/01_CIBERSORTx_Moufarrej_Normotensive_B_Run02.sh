...
for basis_path in "${basis_paths[@]}"; do
    (
        ...
        # Extract the base directory name (last directory name in path) and append -Moufarrej_Normotensive2
        output_dir_name=$(basename "$directory_name" | sed 's/^CIBERSORTx-Matrix_//')-Moufarrej_Normotensive2
        ...
        # Copy the GSE192902_Normotensive_preQC_filtered_counts_UniquePatient_SpecificSample_part2.txt file to the appropriate directory
        echo "Copying GSE192902_Normotensive_preQC_filtered_counts_UniquePatient_SpecificSample_part2.txt to COO-Decon-Published/${directory_name}"
        cp COO-Decon-Published/Moufarrej_Pre-Eclampsia/GSE192902_Normotensive_preQC_filtered_counts_UniquePatient_SpecificSample_part2.txt COO-Decon-Published/${directory_name}
    
        # Run the Docker command for deconvolution
        echo "Running Docker command with mixture file GSE192902_Normotensive_preQC_filtered_counts_UniquePatient_SpecificSample_part2.txt and signature matrix $file_name"
        singularity exec \
            ...
            --mixture /src/data/GSE192902_Normotensive_preQC_filtered_counts_UniquePatient_SpecificSample_part2.txt --sigmatrix /src/data/${file_name} --verbose TRUE
