...
for basis_path in "${basis_paths[@]}"; do
    (
        ...
        # Extract the base directory name (last directory name in path) and append -Chalasani_NAFLD
        output_dir_name=$(basename "$directory_name" | sed 's/^CIBERSORTx-Matrix_//')-Chalasani_NAFLD
        ...
        # Copy the All-NAFLD_Counts_v46_Clean_UniquePatients_CPM.txt file to the appropriate directory
        echo "Copying All-NAFLD_Counts_v46_Clean_UniquePatients_CPM.txt to COO-Decon-Published/${directory_name}"
        cp COO-Decon-Published/Chalasani_Fatty-Liver/All-NAFLD_Counts_v46_Clean_UniquePatients_CPM.txt COO-Decon-Published/${directory_name}
    
        # Run the Docker command for deconvolution
        echo "Running Docker command with mixture file All-NAFLD_Counts_v46_Clean_UniquePatients_CPM.txt and signature matrix $file_name"
        singularity exec \
            ...
            --mixture /src/data/All-NAFLD_Counts_v46_Clean_UniquePatients_CPM.txt --sigmatrix /src/data/${file_name} --verbose TRUE
