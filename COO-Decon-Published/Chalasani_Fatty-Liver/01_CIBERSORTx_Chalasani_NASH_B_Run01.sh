...
for basis_path in "${basis_paths[@]}"; do
    (
        ...
        output_dir_name=$(basename "$directory_name" | sed 's/^CIBERSORTx-Matrix_//')-Chalasani_NASH-01
        ...
        # Copy the All-NASH_Counts_v46_Clean_UniquePatients_CPM_part1.txt file to the appropriate directory
        echo "Copying All-NASH_Counts_v46_Clean_UniquePatients_CPM_part1.txt to COO-Decon-Published/${directory_name}"
        cp COO-Decon-Published/Chalasani_Fatty-Liver/All-NASH_Counts_v46_Clean_UniquePatients_CPM_part1.txt COO-Decon-Published/${directory_name}
    
        # Run the Docker command for deconvolution
        echo "Running Docker command with mixture file All-NASH_Counts_v46_Clean_UniquePatients_CPM_part1.txt and signature matrix $file_name"
        singularity exec \
            ...
            --mixture /src/data/All-NASH_Counts_v46_Clean_UniquePatients_CPM_part1.txt --sigmatrix /src/data/${file_name} --verbose TRUE
