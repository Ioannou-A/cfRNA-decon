...
for basis_path in "${basis_paths[@]}"; do
    (
        ...
        # Extract the base directory name (last directory name in path) and append -Toden-NCI
        output_dir_name=$(basename "$directory_name" | sed 's/^CIBERSORTx-Matrix_//')-Toden-NCI
        ...
        # Copy the All-NCI_Counts_v46_Clean_UniquePatients_CPM.txt file to the appropriate directory
        echo "Copying All-NCI_Counts_v46_Clean_UniquePatients_CPM.txt to COO-Decon-Published/${directory_name}"
        cp COO-Decon-Published/Toden_AD/All-NCI_Counts_v46_Clean_UniquePatients_CPM.txt COO-Decon-Published/${directory_name}
    
        # Run the Docker command for deconvolution
        echo "Running Docker command with mixture file All-NCI_Counts_v46_Clean_UniquePatients_CPM.txt and signature matrix $file_name"
        singularity exec \
            ...
            --mixture /src/data/All-NCI_Counts_v46_Clean_UniquePatients_CPM.txt --sigmatrix /src/data/${file_name} --verbose TRUE
