...
for basis_path in "${basis_paths[@]}"; do
    (
        ...
        # Extract the base directory name (last directory name in path) and append -Toden-AD
        output_dir_name=$(basename "$directory_name" | sed 's/^CIBERSORTx-Matrix_//')-Toden-AD
        echo "Output directory name: $output_dir_name"
        mkdir -p COO-Decon-Published/Toden_AD/${output_dir_name}

        # Extract the final .txt file name
        file_name=$(basename "$basis_path")
        echo "Extracted file name: $file_name"

        # Copy the All-AD_Counts_v46_Clean_UniquePatients_CPM.txt file to the appropriate directory
        echo "Copying All-AD_Counts_v46_Clean_UniquePatients_CPM.txt to COO-Decon-Published/${directory_name}"
        cp COO-Decon-Published/Toden_AD/All-AD_Counts_v46_Clean_UniquePatients_CPM.txt COO-Decon-Published/${directory_name}
    
        # Run the Docker command for deconvolution
        echo "Running Docker command with mixture file All-AD_Counts_v46_Clean_UniquePatients_CPM.txt and signature matrix $file_name"
        singularity exec \
            -B COO-Decon-Published/${directory_name}:/src/data \
            -B COO-Decon-Published/Toden_AD/${output_dir_name}:/src/outdir \
            fractions_latest.sif /src/CIBERSORTxFractions \
            --username "$CIBERSORTX_USERNAME" --token "$CIBERSORTX_TOKEN" \
            --mixture /src/data/All-AD_Counts_v46_Clean_UniquePatients_CPM.txt --sigmatrix /src/data/${file_name} --verbose TRUE
