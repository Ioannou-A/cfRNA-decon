...
bulk_files=(
  "COO-Decon-Published/Loy_Inflammatory-Syndromes/GSE255555_pedInflam_filtered_counts_CPM_GeneNames.txt"
)
...
output_dir="COO-Decon-Published/Loy_Inflammatory-Syndromes/Decon-COO-Results_Loy_Inflammatory-Syndromes/"
mkdir -p "$output_dir"
...
for bulk in "${bulk_files[@]}"; do
  for sc in "${sc_refs[@]}"; do
    ...
    Rscript COO-Decon-Published/Loy_Inflammatory-Syndromes/01_BayesPrism_A_Run.R "$bulk" "$sc" "$output_file"
