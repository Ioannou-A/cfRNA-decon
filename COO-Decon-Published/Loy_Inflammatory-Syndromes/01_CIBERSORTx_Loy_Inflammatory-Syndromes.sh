...
for basis_path in "${basis_paths[@]}"; do
    (
        ...
        output_dir_name="Decon-COO-Results_Loy_Inflammatory-Syndromes"
        ...
        # Copy the GSE255555_pedInflam_filtered_counts_CPM_GeneNames.txt file to the appropriate directory
        echo "Copying GSE255555_pedInflam_filtered_counts_CPM_GeneNames.txt to COO-Decon-Published/${directory_name}"
        cp COO-Decon-Published/Loy_Inflammatory-Syndromes/GSE255555_pedInflam_filtered_counts_CPM_GeneNames.txt COO-Decon-Published/${directory_name}
    
        # Run the Docker command for deconvolution
        echo "Running Docker command with mixture file GSE255555_pedInflam_filtered_counts_CPM_GeneNames.txt and signature matrix $file_name"
        singularity exec \
            -B COO-Decon-Published/${directory_name}:/src/data \
            -B COO-Decon-Published/Loy_Inflammatory-Syndromes/${output_dir_name}:/src/outdir \
            fractions_latest.sif /src/CIBERSORTxFractions \
            --username "$CIBERSORTX_USERNAME" --token "$CIBERSORTX_TOKEN" \
            --mixture /src/data/GSE255555_pedInflam_filtered_counts_CPM_GeneNames.txt --sigmatrix /src/data/${file_name} --verbose TRUE
