...
for data_path in data_paths:
    ...
    for basis_path in basis_paths:
        ...
        # Extract basis matrix identifier, e.g. "2Median_300_500"
        basis_dir = os.path.dirname(basis_path)
        basis_id = os.path.basename(basis_dir).replace("CIBERSORTx-TOO-Matrix_", "")

        # Desired output:
        # Decon-Results_20250616_All-Tissues-NoDup_Random_*_percent_removed_CPM_2Median_300_500/
        output_dir_name = f"Decon-Results_{data_basename}_{basis_id}/"
        output_dir = os.path.join(base_output_dir, output_dir_name)
        os.makedirs(output_dir, exist_ok=True)

        print(f"Running NNLS: Data={data_path} | Basis={basis_path} -> Output={output_dir}")
