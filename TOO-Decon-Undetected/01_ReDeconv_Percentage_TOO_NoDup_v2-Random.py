...
output_folder = 'TOO-Decon-Undetected/ReDeconv-TOO_v2-NoDup-Degradation/'

# Paths to bulk RNA-seq input files
mixture_paths = [
...
exp_files = sorted(glob.glob('TOO-Matrix/TOO-Matrices_ReDeconv/*Sampling10-Unique_ReDeconv_Renamed.txt'))
...
for bulk_RNAseq_file in mixture_paths:
    bulk_prefix = os.path.basename(bulk_RNAseq_file).replace('.txt', '')

    for fn_meta, fn_exp, prefix in file_pairs:
        print(f'\nProcessing pair: {prefix} with bulk: {bulk_prefix}')
        ...
        for L_topNo in L_topNo_list:
            # Updated tag to include prefix to prevent overwriting
            tag = f'{prefix}_{bulk_prefix}_Top{L_topNo}'
            ...
            fn_percentage_save = f'{output_folder}{tag}_ReDeconv_results.txt'
