...
class DANN(object):
    ...
    def DANN_model(self, celltype_num):
        ...
        self.encoder_da = nn.Sequential(EncoderBlock(feature_num, 1024, 0), 
                                        EncoderBlock(1024, 512, 0.3))

        self.predictor_da = nn.Sequential(EncoderBlock(512, 256, 0.2), 
                                          nn.Linear(256, celltype_num), 
                                          nn.Softmax(dim=1))
        
        self.discriminator_da = nn.Sequential(EncoderBlock(512, 256, 0.2), 
                                              nn.Linear(256, 1), 
                                              nn.Sigmoid())
