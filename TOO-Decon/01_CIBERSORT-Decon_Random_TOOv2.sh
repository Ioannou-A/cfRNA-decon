...
for basis_path in "${basis_paths[@]}"; do
        ...
        # Extract the base directory name (last directory name in path) and append -Random_TOOv2
        output_dir_name=$(basename "$directory_name" | sed 's/^CIBERSORTx-TOO-Matrix/Decon-Results/')-Random_TOOv2_1000
        ...
        cp TOO-Decon/20250616_All-Tissues-NoDup_Random_Simulated_v2_Counts.txt ${directory_name}
        ...
        singularity exec -B ${directory_name}:/src/data -B ${output_dir_name}:/src/outdir fractions_latest.sif /src/CIBERSORTxFractions \
           --username "$CIBERSORTX_USERNAME" --token "$CIBERSORTX_TOKEN" --mixture /src/data/20250616_All-Tissues-NoDup_Random_Simulated_v2_Counts.txt --sigmatrix /src/data/${file_name} --verbose TRUE
