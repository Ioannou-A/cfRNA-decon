...
NCI_dir="Raw-Published/Toden_AD/All-NCI-Aligned"
gtf="Raw-Published/gencode.v46.annotation.gtf"

featureCounts -p -g gene_name -t exon -a "$gtf" -Q 10 -T 10 -F GTF -o "$NCI_dir/All-NCI_Counts_v46.txt" "$NCI_dir"/*nodup.bam

# 2) Clean header + drop cols 2–6 -> counts-only table
clean_Counts_v46="$NCI_dir/All-NCI_Counts_v46_Clean.txt"
tail -n +2 "$NCI_dir/All-NCI_Counts_v46.txt" | \
  cut -f1,7- | \
  awk 'BEGIN{OFS="\t"}
       NR==1 {
