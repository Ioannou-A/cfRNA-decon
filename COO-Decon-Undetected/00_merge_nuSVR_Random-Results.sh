...
for src_dir in Decon-COO-Results_Degradation_v4_*_percent_removed_nuSVR; do
    [[ -d "$src_dir" ]] || continue

    # Extract the percent part 
    percent=$(echo "$src_dir" | sed -E 's/.*_v4_([0-9]+)_percent_removed_nuSVR/\1/')
    target_dir="TSP-BDa_Outer_300_1500_10_Random_v2C_NotDetected_${percent}_percent_removed_CPM"
    mkdir -p "$target_dir"
...
for file in TSP-BDa_Outer_300_1500_10_Random_v2C_NotDetected_*_percent_removed_CPM/nuSVR_Counts_*.txt; do
  [[ -f "$file" ]] || continue
