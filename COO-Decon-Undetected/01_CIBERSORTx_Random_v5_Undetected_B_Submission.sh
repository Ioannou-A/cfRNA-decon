...
mixture_dir="COO-Decon-Undetected/Data"

# Generate list of all mixture files and pick one by TASKID
mixture_file=$(ls ${mixture_dir}/Random_v2C_NotDetected_*_percent_removed_CPM_part*.txt | sort | sed -n "${TASKID}p")
...
outdir="COO-Decon-Undetected/${output_dir_name}_${mixture_base}"
mkdir -p "$outdir"
