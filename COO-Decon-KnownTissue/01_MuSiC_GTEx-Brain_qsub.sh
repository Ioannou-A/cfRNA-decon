...
# Define directories
MIXTURE_DIR="COO-Decon-KnownTissue"
RESULTS_DIR="${MIXTURE_DIR}/Results-GTEx-Brain"

mkdir -p "$RESULTS_DIR"

# Define mixture file
MIXTURE="${MIXTURE_DIR}/20250830_GeneId_Brain_50PerRegion-Unique.OnlyInBrainRemapped.txt"
...
for ref in COO-Matrix/TSP*100each_seed42_filtered.txt; do
    ...
    if [[ -f "$meta" ]]; then
        ...
        # Run the R script
        OUTPUT_FILE="${RESULTS_DIR}/$(basename "$ref" .txt)_GTEx-Brain_MuSiC.txt"
        Rscript COO-Decon-KnownTissue/01_run_MuSiC-2.R "$ref" "$meta" "$MIXTURE" "$OUTPUT_FILE"
    else
