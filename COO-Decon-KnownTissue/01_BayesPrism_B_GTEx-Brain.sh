...
bulk_files=(
  "COO-Decon-KnownTissue/20250830_GeneId_Brain_50PerRegion-Unique.OnlyInBrainRemapped.txt"
)
...
sc_refs=(
  ...
  "COO-Matrix/TSP-BDa_Outer_100each_seed42_filtered.txt"
  "COO-Matrix/TSP-HBA_Inner_100each_seed42_filtered.txt"
)
BayesPrism_B_GTEx-Brain.
# Define the output directory and create it if it doesn't exist
module load anaconda
conda activate bayes

output_dir="COO-Decon-KnownTissue/BayesPrism-Deconvolutions"
mkdir -p "$output_dir"
...
for bulk in "${bulk_files[@]}"; do
  for sc in "${sc_refs[@]}"; do
    ...
    Rscript COO-Decon-KnownTissue/01_BayesPrism_A_Run.R "$bulk" "$sc" "$output_file"
